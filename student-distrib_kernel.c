...
//macro used to determine whether or not the tests will be run
#define RUN_TESTS 0
...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        while (mod_count < mbi->mods_count) {
            ...
            //get module address for filesystem
    ...
    //set pid 0 to available while interrupts are masked to ensure that the shell we are about to execute gets assigned to pid 0
    pidAvailable[PID0] = AVAILABLE; //0 corresponds with available
